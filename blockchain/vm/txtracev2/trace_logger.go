...
var logger = log.NewModuleLogger(log.BlockchainVmOeTracer)
...
func memorySlice(memory []byte, offset, size uint64) []byte {
	...
	if offset+size < offset {
		log.Root().Warn("Tracer accessed out of bound memory", "offset", offset, "size", size)
		return nil
	}
	if len(memory) < int(offset+size) {
		log.Root().Warn("Tracer accessed out of bound memory", "available", len(memory), "offset", offset, "size", size)
		return nil
	}
...
func (ot *OeTracer) CaptureState(env *vm.EVM, pc uint64, op vm.OpCode, gas, cost, ccLeft, ccOpcode uint64, scope *vm.ScopeContext, depth int, err error) {
	switch op {
	case vm.CREATE, vm.CREATE2:
...
func (ot *OeTracer) checkCanTransfer(addr common.Address, value *uint256.Int) error {
	if value.Sign() != 0 && !ot.env.Context.CanTransfer(ot.env.StateDB, addr, value.ToBig()) {
...
func (ot *OeTracer) CaptureFault(env *vm.EVM, pc uint64, op vm.OpCode, gas, cost, ccLeft, ccOpcode uint64, scope *vm.ScopeContext, depth int, err error) {
}
...
func (ot *OeTracer) PersistTrace() {
	if ot.store != nil {
		...
		if err != nil {
			logger.Error("Failed to encode tx trace", "txHash", ot.outPutTraces.TransactionHash.String(), "err", err.Error())
			return
		}
		if err := ot.store.WriteTxTrace(context.Background(), ot.outPutTraces.TransactionHash, tracesBytes); err != nil {
			logger.Error("Failed to persist tx trace to database", "txHash", ot.outPutTraces.TransactionHash.String(), "err", err.Error())
			return
		}
	}
}
